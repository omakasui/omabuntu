...
output=$("$CLI" commands)
assert_output_contains "commands lists documented commands" "$output" "omakub theme set <theme-name>"
...
"$CLI" commands --all --json | jq -e '.commands[] | select(.route == "omakub dev add migration" and .summary != "undocumented")' >/dev/null
...
assert_output_contains "restart group includes all restart commands" "$output" "omakub restart opencode"

output=$("$CLI" menu --help)
assert_output_contains "menu group includes share fallback route" "$output" "omakub menu share"

output=$("$CLI" share)
assert_output_contains "bare required-arg alias renders CLI help" "$output" "Usage:"
assert_output_contains "bare share help uses canonical route" "$output" "omakub share <clipboard|file|folder> [path...]"

output=$("$CLI" menu share)
assert_output_contains "bare required-arg filename route renders CLI help" "$output" "omakub share <clipboard|file|folder> [path...]"

output=$("$CLI" branch set)
assert_output_contains "bare required-choice route renders CLI help" "$output" "omakub branch set <main|dev>"
...
for command in commands:
  ...
  stem = binary.removeprefix('omakub-')
  group = stem.split('-', 1)[0]
  filename_route = 'omakub ' + stem.replace('-', ' ')
...
assert_output_contains "theme set help names binary" "$output" "omakub-theme-set"

output=$(timeout 5 "$CLI" update --help)
assert_output_contains "mutating command help does not execute target" "$output" "omakub-update"
assert_output_contains "root command help shows related child commands" "$output" "omakub update perform"

output=$("$CLI" pkg add --help)
assert_output_contains "fallback route resolves to curated metadata" "$output" "omakub-pkg-add"
assert_output_contains "fallback route shows canonical route" "$output" "omakub install package <packages...>"
...
while IFS= read -r binary_path; do
  ...
  grep -q '^# omakub:summary=' <<<"$header" || fail "metadata summary is present: $binary_path"
  ! grep -q '^# omakub:binary=' <<<"$header" || fail "metadata does not repeat inferred binary: $binary_path"
  ! grep -q '^# omakub:args=$' <<<"$header" || fail "metadata does not include empty args: $binary_path"
  ! grep -Eq '^# omakub:(legacy|usage|visibility|mutates|interactive)=' <<<"$header" || fail "metadata avoids removed fields: $binary_path"
  ! grep -Eq '^# omakub:requires-sudo=false$' <<<"$header" || fail "metadata omits false booleans: $binary_path"
done < <(find "$ROOT/bin" -maxdepth 1 -type f -executable -name 'omakub-*' | sort)
pass "all executable bins have slim self-documenting metadata"

TMPDIR=$(mktemp -d)
ln -s "$CLI" "$TMPDIR/omakub"

{
  printf '#!/bin/bash\n\n'
  printf '# ordinary comments are fine\n'
  printf '# omakub:this malformed line should be ignored\n'
  printf '# omakub:group=weird\n'
  printf '# omakub:name=test\n'
  printf '# omakub:summary=Survives malformed metadata comments\n'
  printf '# omakub:made-up=value\n'
  printf 'echo weird-ok\n'
} >"$TMPDIR/omakub-weird-test"
chmod +x "$TMPDIR/omakub-weird-test"

{
  printf '#!/bin/bash\n\n'
  printf '# a partial metadata header should not destroy fallback routing\n'
  printf '# omakub:summary=Partial metadata keeps inferred route\n'
  printf '# omakub:made-up=value\n'
  printf 'echo partial-ok\n'
} >"$TMPDIR/omakub-partial-meta-test"
chmod +x "$TMPDIR/omakub-partial-meta-test"

{
  printf '#!/bin/bash\n\n'
  printf 'echo body-metadata-ok\n'
  printf '# omakub:group=wrong\n'
  printf '# omakub:name=wrong\n'
} >"$TMPDIR/omakub-body-metadata-test"
chmod +x "$TMPDIR/omakub-body-metadata-test"

"$TMPDIR/omakub" commands --all --json | jq -e '.commands[] | select(.route == "omakub weird test" and .summary == "Survives malformed metadata comments")' >/dev/null
pass "unknown metadata values are non-fatal"

"$TMPDIR/omakub" commands --all --json | jq -e '.commands[] | select(.route == "omakub partial meta test" and .summary == "Partial metadata keeps inferred route")' >/dev/null
pass "partial metadata keeps inferred fallback route"

"$TMPDIR/omakub" commands --all --json | jq -e '.commands[] | select(.route == "omakub body metadata test" and .summary == "Run the body metadata test command")' >/dev/null
pass "metadata-looking comments after script body are ignored"

output=$("$TMPDIR/omakub" weird test)
assert_output_contains "temporary metadata command dispatches" "$output" "weird-ok"

output=$("$TMPDIR/omakub" partial meta test)
assert_output_contains "partial metadata command dispatches" "$output" "partial-ok"

output=$("$TMPDIR/omakub" body metadata test)
